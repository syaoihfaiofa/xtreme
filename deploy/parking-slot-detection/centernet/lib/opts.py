...
class opts(object):
  replace_dcn_import = False

  def __init__(self):
    self.parser = argparse.ArgumentParser()
    # basic experiment setting
    self.parser.add_argument('task', default='ctdet',
                             help='ctdet | ddd | multi_pose | exdet | ctdet_voc | my_ctdet_voc')
    self.parser.add_argument('--dataset', default=None,
                             help='coco | kitti | coco_hp | pascal | my_pascal')
    self.parser.add_argument('--labels_file', default=None) 
    self.parser.add_argument('--dataset_name', default=None)
    self.parser.add_argument('--split_set', default=None)
    ...
    self.parser.add_argument('--deconv_type', type=int, default=0,
                             help='deconv layer type'
                                  '0 for deconv with group8'
                                  '1 for deconv with no group'
                                  '2 for resize+conv with group8')
    ...
    self.parser.add_argument('--batch_size', type=int, default=48,
                                help='batch size')
    self.parser.add_argument('--valid_batch_size_per_gpu', type=int, default=48,
                            help='batch size')
    ...
    self.parser.add_argument('--use_amp',action='store_true',
                            help='weather use the amp to train the networks')
    ...
    self.parser.add_argument('--CUDNN_BENCHMARK', type=bool, default=True)
    self.parser.add_argument('--CUDNN_DETERMINISTIC', type=bool, default=False)
    self.parser.add_argument('--CUDNN_ENABLED', type=bool, default=True)
    ...
    self.parser.add_argument('--vflip', type = float, default=0,
                             help='probability of applying vflip augmentation.')
    self.parser.add_argument('--no_color_aug', action='store_true',
                             help='not use the color augmenation '
                                  'from CornerNet')
    self.parser.add_argument('--cbgs', default='none', choices=['none', 'v1', 'v2'],
                             help='use Class Balanced Group Sampling for data loading, v1: original CBGS, v2: improved CBGS')
    # multi_pose
    ...
    self.parser.add_argument('--not_reg_hps', action='store_true',
                             help='not regression keypoints.')
    self.parser.add_argument('--eccentric', action='store_true',
                             help='use eccentric point instead of center point.')
    self.parser.add_argument('--no_standard_mask', action='store_true',
                             help='do not use mask according to label standards')
    
    self.parser.add_argument('--mtl', type=str, 
  ...
  def parse(self, args=''):
    ...
    opt.reg_hps = not opt.not_reg_hps
  ...
  def update_dataset_info_and_set_heads(self, opt):
    ...
    dataset.labels = labels.Labels(opt.labels_file)

    input_h, input_w = dataset.default_resolution
    opt.mean, opt.std = dataset.mean, dataset.std
    opt.num_classes = dataset.labels.num_classes
    ...
    if opt.task == 'exdet':
    ...
    elif opt.task == 'ctdet' or opt.task == 'ctdet_voc' or opt.task == 'my_ctdet_voc':
    ...
    elif opt.task == 'my_multi_pose':
      # assert opt.dataset in ['coco_hp']
      opt.flip_idx = dataset.flip_idx
      opt.num_joints = dataset.num_joints
      
      # Calculate class counts for each group
      num_basic_cls = dataset.labels.get_group_count("")
      num_corner_cls = dataset.labels.get_group_count("corner")
      num_mask_cls = dataset.labels.get_group_count("mask")
      
      # Build heads based on available classes
      opt.heads = {}
      
      # Basic detection heads (only if basic classes exist)
      if num_basic_cls > 0:
        opt.heads['hm'] = num_basic_cls
        if opt.reg_bbox:
          opt.heads.update({'wh': 2})
        if opt.reg_hps:
          opt.heads.update({'hps': opt.num_joints * 2})
        if opt.reg_offset:
          opt.heads.update({'reg': 2})
        if opt.hm_hp:
          opt.heads.update({'hm_hp': opt.num_joints})
      ...
      # Corner detection heads (only if corner classes exist)
      if num_corner_cls > 0:
        opt.heads['corner_hm'] = num_corner_cls
        opt.heads['corner_hps'] = 3 * 2
      
      # Mask segmentation head (only if mask classes exist)
      # Output channels = num_mask_cls + 1 (including background)
      if num_mask_cls > 0:
        opt.heads['seg'] = num_mask_cls + 1  # +1 for background class

      if opt.dataset_name is None:
        opt.dataset_name = 'my_voc'
    else:
  ...
  def init(self, args=''):
    default_dataset_info = {
      'ctdet': {'dataset': 'coco', 'labels_file': 'labels_coco.txt'},
      'ctdet_voc': {'dataset': 'pascal', 'labels_file': 'labels_pascal.txt'},
      'my_ctdet_voc': {'dataset': 'my_pascal', 'labels_file': 'labels_my_pascal.txt'},
      'exdet': {'dataset': 'coco', 'labels_file': 'labels_coco.txt'},
      'multi_pose': {'dataset': 'coco_hp', 'labels_file': 'labels_coco_hp.txt'},
      'my_multi_pose': {'dataset': 'my_xml_hp', 'labels_file': 'labels_my_coco_hp.txt'},
      'ddd': {'dataset': 'kitti', 'labels_file': 'labels_kitti.txt'},
    }
    class Struct:
    ...
    opt = self.parse(args)
    default_info = default_dataset_info[opt.task]
    if opt.dataset is None:
      opt.dataset = default_info['dataset']
    if opt.labels_file is None:
      opt.labels_file = os.path.join('lib', 'datasets', default_info['labels_file'])
