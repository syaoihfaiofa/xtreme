...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', headless=False, 
    ...
    #self.names = dataset_class.class_names[1:] # exclude background
    if dataset == 'coco_hp':
      #self.num_joints = dataset_class.num_joints
      ...
      self.colors_hp = [(255, 0, 255), (255, 0, 0), (0, 0, 255), 
        ...
        (255, 0, 0), (0, 0, 255), (255, 0, 0), (0, 0, 255),
        (255, 0, 0), (0, 0, 255)]
    elif dataset == 'my_coco_hp' or dataset == 'my_xml_hp':
      #self.num_joints = dataset_class.num_joints
      self.edges = [[0, 1], [1, 2], [2, 3]]
      self.ec = [(255, 0, 0), (0, 0, 255), (255, 0, 0), (0, 0, 255)]
      self.colors_hp = [(255, 0, 0), (0, 0, 255), (255, 0, 0), (0, 0, 255)]
    elif dataset == 'coco':
      pass
    elif dataset == 'pascal':
      pass
    elif dataset == 'my_pascal':
      pass
    # elif dataset == 'gta':
  ...
  def show_all_imgs(self, pause=False, time=0):
    if self.headless:
      print ('saving results', self.file_save_id)
      for i, v in self.imgs.items():
        cv2.imwrite('{:05d}_{}.png'.format(self.file_save_id, i), v)
      self.file_save_id += 1
    elif not self.ipynb:
      for i, v in self.imgs.items():
        cv2.imshow('{}'.format(i), v)
      self.key_pressed = cv2.waitKey(0 if pause else 1)
