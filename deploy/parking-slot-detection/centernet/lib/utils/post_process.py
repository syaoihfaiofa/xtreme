...
def multi_pose_post_process(dets, c, s, h, w, num_classes, num_joints):
  ...
  for i in range(dets.shape[0]):
    top_preds = {}
    for t in range(2):
      dets[i, :, 2*t:2*t+2] = transform_preds(
            dets[i, :, 2*t:2*t+2], c[i], s[i], (w, h))
    for t in range(num_joints):
      dets[i, :, 5+2*t:5+2*t+2] = transform_preds(
            dets[i, :, 5+2*t:5+2*t+2], c[i], s[i], (w, h))
    
    #bbox = transform_preds(dets[i, :, :4].reshape(-1, 2), c[i], s[i], (w, h))
    #pts = transform_preds(dets[i, :, 5:5+num_joints*2].reshape(-1, 2), c[i], s[i], (w, h))
    classes = dets[i, :, -1]
    for j in range(num_classes):
      inds = (classes == j)
      top_preds[j + 1] = np.concatenate([
        ...
        dets[i, inds, 5:5+num_joints*2].astype(np.float32)], axis=1).tolist()
      # top_preds[j + 1] = np.concatenate(
      #   [bbox.reshape(-1, 4), dets[i, :, 4:5], 
      #   pts.reshape(-1, num_joints*2)], axis=1).astype(np.float32).tolist()
    ret.append(top_preds)
  return ret
