...
from mtl.min_norm_solvers import MinNormSolver, gradient_normalizers
...
class NashMTL(WeightMethod):
    ...
    def solve_optimization(self, gtg: np.array):
        ...
        # Weizhe Liu: when data is paritally annotated, for example task 2 has no labeled data, the gradiant of task 2 will be zero
        # but the solver expects non-zero values when calculate log(), so we add a small value to avoid NaN
        epsilon = 1e-8
        gtg_reg = gtg + epsilon * np.eye(gtg.shape[0]) 

        self.G_param.value = gtg_reg
        self.normalization_factor_param.value = self.normalization_factor

        alpha_t = self.prvs_alpha
        for iter_step in range(self.optim_niter):
            ...
            if self._stop_criteria(gtg_reg, alpha_t):
                break
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        if (self.step % self.update_weights_every) == 0:
            ...
            # Check for NaN/Inf in GTG or zero normalization_factor
            device = losses[0].device if len(losses) > 0 else torch.device('cpu')
            if self.normalization_factor.item() == 0 or torch.isnan(GTG).any() or torch.isinf(GTG).any():
                # Fallback to uniform weights if GTG is invalid
                alpha = np.ones(len(losses)) / len(losses)
                alpha = torch.from_numpy(alpha).to(device)
            else:
                GTG = GTG / self.normalization_factor.item()
                # Check for NaN/Inf after normalization
                GTG_np = GTG.cpu().detach().numpy()
                if np.isnan(GTG_np).any() or np.isinf(GTG_np).any():
                    # Fallback to uniform weights if GTG is invalid
                    alpha = np.ones(len(losses)) / len(losses)
                    alpha = torch.from_numpy(alpha).to(device)
                else:
                    ...
                    alpha = torch.from_numpy(alpha).to(device)
                    # Clip alpha values to reasonable range to prevent extreme weights
                    # This helps when loss scales differ significantly
                    max_alpha = 100.0  # Reasonable upper bound
                    alpha = torch.clamp(alpha, min=0.0, max=max_alpha)
                    # Renormalize to ensure they sum to reasonable value
                    alpha_sum = alpha.sum().item()
                    if alpha_sum > max_alpha * len(losses):
                        alpha = alpha / alpha_sum * max_alpha * len(losses)
