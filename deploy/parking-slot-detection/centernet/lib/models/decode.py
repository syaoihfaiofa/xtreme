...
def multi_pose_decode_base(
  ...
  if wh is not None:
    ...
    bboxes = torch.cat([xs, ys, xs, ys], dim=2)

  if kps is None: # fill all 4 kps with center if no kps predict
    kps = torch.zeros((batch, K, 4 * 2), device=heat.device) # avoid hardcode num_joints to 4
    kps = torch.cat([xs, ys, xs, ys, xs, ys, xs, ys], dim=2)
  
  # disable by default, because it would match to nearby points incorrectly.
  # especially for PSD, which would contain hp points near each other, for adjacent parkingpaces
  if False: 
  ...
  if False:
    # restore score from bbox center, when use eccentric point
    xs_int = torch.round(xs).to(torch.int)
    ys_int = torch.round(ys).to(torch.int)
    xs_int = torch.clamp(xs_int, min=0, max=width-1)
    ys_int = torch.clamp(ys_int, min=0, max=height-1)
    
    clses_int = clses.view(batch, K).long()
    xs_int = xs_int.view(batch, K).long()
    ys_int = ys_int.view(batch, K).long()
    batch_indices = torch.arange(batch).unsqueeze(1).expand(-1, K)
    #ct_scores = hm_ct[batch_indices, clses_int, ys_int, xs_int]

    ct_scores = torch.zeros(batch, K, device=scores.device)
    for b in range(batch):
        for k in range(K):
            ...
            x_min = torch.clamp(x-1, min=0, max=width)
            x_max = torch.clamp(x+2, min=0, max=width)
            y_min = torch.clamp(y-1, min=0, max=height)
            y_max = torch.clamp(y+2, min=0, max=height)

            neighborhood = hm_ct[b, c, y_min:y_max, x_min:x_max]
            #print ('neighborhood idx', b, c, y_min,y_max, x_min,x_max, 'xy', x, y)
            #print ('neighborhood', neighborhood)
            
            # Find the max score in the neighborhood
            ct_scores[b, k] = neighborhood.max()

    ct_scores = ct_scores.view(batch, K, 1)

    #print ('scores', scores[... , :10, :])
    #print ('ct_scores', ct_scores[... , :10, :])

    # score should not be too low which means invalid det
    # and ct_score should not be too larger than score, avoid boosting a very low score to very high
    condition = (scores > 0.1) & (ct_scores > scores) & (ct_scores < scores * 2)
    updated_scores = torch.where(condition, ct_scores, scores)
  
    

    # hmct_print = hm_ct.cpu().numpy()[0,0,...]
    # np.set_printoptions(precision=2, suppress=True, linewidth=140, threshold=np.inf)
    # hmct_print[hmct_print < 0.2] = 0
    # print('hmct',hmct_print)
    # print('max',np.max(hmct_print))

    #print ('scores_new', updated_scores[... , :10, :])
    #print('valid', updated_scores[scores>0.6])
...
#   hm_print = heat.cpu().numpy()[0,0,...]
#   np.set_printoptions(precision=2, suppress=True, linewidth=140, threshold=np.inf)
#   hm_print[hm_print < 0.2] = 0
#   print('hm', hm_print)
#   print('max',np.max(hm_print))

  return detections

def multi_pose_decode_corner(hm=None, hps=None, K=100):
  ...
  if hps is not None:
    ...
    hps[..., ::2] += xs.expand(batch, K, num_joints)
    hps[..., 1::2] += ys.expand(batch, K, num_joints) 
...
def multi_pose_decode(
    heat, wh, kps, reg=None, hm_hp=None, hp_offset=None, 
    corner_hm=None, corner_hps=None, seg=None, K=100):
  
  dets = {}
  if heat is not None:
    dets[''] = multi_pose_decode_base(heat, wh, kps, reg, hm_hp, hp_offset, K)

  if corner_hm is not None:
    dets['corner'] = multi_pose_decode_corner(corner_hm, corner_hps, K)

  if seg is not None:
    # seg: (batch, num_classes, H, W) -> (batch, H, W) class indices
    seg_pred = seg.argmax(dim=1)  # (batch, H, W)
    dets['seg'] = seg_pred
