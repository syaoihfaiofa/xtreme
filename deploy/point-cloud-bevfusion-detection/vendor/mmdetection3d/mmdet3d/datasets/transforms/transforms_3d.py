...
@TRANSFORMS.register_module()
class FilterBoxesWithoutPoints(BaseTransform):
    """Remove GT boxes that contain too few points in the current point cloud.
    ...
    Args:
        min_num_points (int): Minimum number of points required to keep a box.
    """

    def __init__(self, min_num_points: int) -> None:
        if min_num_points < 1:
            raise ValueError(
                f'min_num_points must be at least 1, got {min_num_points}.')
        self.min_num_points = min_num_points

    def transform(self, input_dict: dict) -> dict:
        """Filter boxes without enough interior points."""
        gt_bboxes_3d = input_dict['gt_bboxes_3d']
        gt_labels_3d = input_dict['gt_labels_3d']
        if len(gt_bboxes_3d) == 0:
            return input_dict

        points = input_dict['points'].numpy()
        boxes = gt_bboxes_3d.tensor.numpy()
        point_indices = box_np_ops.points_in_rbbox(
            points, boxes, z_axis=2, origin=(0.5, 0.5, 0))
        num_points_in_gt = point_indices.sum(axis=0)
        keep_mask = num_points_in_gt >= self.min_num_points
        keep_mask_tensor = torch.from_numpy(keep_mask)

        input_dict['gt_bboxes_3d'] = gt_bboxes_3d[keep_mask_tensor]
        input_dict['gt_labels_3d'] = gt_labels_3d[keep_mask]
        return input_dict
