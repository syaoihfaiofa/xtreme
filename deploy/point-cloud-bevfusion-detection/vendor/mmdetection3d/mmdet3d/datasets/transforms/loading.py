...
def load_ascii_pcd(pts_bytes: bytes, pts_filename: str,
                   load_dim: int) -> np.ndarray:
    """Load an ASCII PCD file and pad its fields to ``load_dim``."""
    try:
        header, point_data = pts_bytes.decode('utf-8').split(
            '\nDATA ascii', maxsplit=1)
    except (UnicodeDecodeError, ValueError) as error:
        raise ValueError(
            f'Expected an ASCII PCD file with a DATA ascii header: '
            f'{pts_filename}') from error

    fields = None
    for header_line in header.splitlines():
        if header_line.startswith('FIELDS '):
            fields = header_line.split()[1:]
            break
    if fields is None:
        raise ValueError(f'PCD file is missing FIELDS metadata: {pts_filename}')
    if len(fields) > load_dim:
        raise ValueError(
            f'PCD file has {len(fields)} fields, exceeding load_dim={load_dim}: '
            f'{pts_filename}')

    points = np.fromstring(point_data, sep=' ', dtype=np.float32)
    if points.size % len(fields) != 0:
        raise ValueError(
            f'PCD point data does not match {len(fields)} fields: '
            f'{pts_filename}')
    points = points.reshape(-1, len(fields))
    if len(fields) < load_dim:
        points = np.pad(
            points, ((0, 0), (0, load_dim - len(fields))), constant_values=0)
    return points


def load_points_file(pts_filename: str, load_dim: int,
                     backend_args: Optional[dict]) -> np.ndarray:
    """Load binary, NumPy, or ASCII PCD point cloud data."""
    try:
        pts_bytes = get(pts_filename, backend_args=backend_args)
        if pts_filename.endswith('.pcd'):
            return load_ascii_pcd(pts_bytes, pts_filename, load_dim)
        return np.frombuffer(pts_bytes, dtype=np.float32)
    except ConnectionError:
        mmengine.check_file_exist(pts_filename)
        if pts_filename.endswith('.npy'):
            return np.load(pts_filename)
        if pts_filename.endswith('.pcd'):
            with open(pts_filename, 'rb') as file_obj:
                return load_ascii_pcd(
                    file_obj.read(), pts_filename, load_dim)
...
class LoadPointsFromMultiSweeps(BaseTransform):
    ...
    def _load_points(self, pts_filename: str) -> np.ndarray:
        ...
        """
        return load_points_file(pts_filename, self.load_dim,
                                self.backend_args)
