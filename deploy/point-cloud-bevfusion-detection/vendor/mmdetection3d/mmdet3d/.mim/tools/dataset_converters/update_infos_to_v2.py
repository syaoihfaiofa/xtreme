...
CUSTOM_NUSCENES_CATEGORY_MAPPING = {
    category_name: category_name
    for category_name in CUSTOM_NUSCENES_CLASSES
}


def get_nuscenes_category_label(category_name: str) -> Optional[int]:
    """Return the label for a supported custom NuScenes category."""
    mapped_category = CUSTOM_NUSCENES_CATEGORY_MAPPING.get(category_name)
    if mapped_category is None:
        return None
    return CUSTOM_NUSCENES_CLASSES.index(mapped_category)


def convert_sweep_to_loader_transform(
        rotation: list[list[float]], translation: list[float]) -> np.ndarray:
    """Return the sweep-to-current transform expected by the point loader."""
    transform = np.eye(4)
    transform[:3, :3] = np.asarray(rotation).T
    transform[:3, 3] = -np.asarray(translation)
    return transform
...
def get_nuscenes_camera_types(info):
    """Return the camera channels recorded in one legacy NuScenes info."""
    return list(info['cams'])


def generate_nuscenes_camera_instances(info, nusc):

    # get bbox annotations for camera
    camera_types = get_nuscenes_camera_types(info)
...
def update_nuscenes_infos(pkl_path, out_dir):
    ...
    camera_types = (get_nuscenes_camera_types(data_list['infos'][0])
                    if data_list['infos'] else [])
    METAINFO = {'classes': CUSTOM_NUSCENES_CLASSES}
    nusc = NuScenes(
    ...
    for i, ori_info_dict in enumerate(
        ...
        for ori_sweep in ori_info_dict['sweeps']:
            ...
            lidar2sensor = convert_sweep_to_loader_transform(
                ori_sweep['sensor2lidar_rotation'],
                ori_sweep['sensor2lidar_translation'])
            temp_lidar_sweep['lidar_points'][
