...
def _box_tensor_to_7d(box_tensor: torch.Tensor) -> torch.Tensor:
    """Use the first 7 box params; drop velocity and extra dims."""
    return box_tensor.reshape(-1)[:7]


def _new_box7(box_tensor: torch.Tensor, ref_box) -> object:
    """Build a 7D box container from a reference box type."""
    return type(ref_box)(box_tensor, box_dim=7, with_yaw=ref_box.with_yaw)
...
def eval_det_cls(pred, gt, iou_thr=None):
    ...
    for img_id in gt.keys():
        ...
        if cur_gt_num != 0:
            ...
            for i in range(cur_gt_num):
                gt_cur[i] = _box_tensor_to_7d(gt[img_id][i].tensor)
            bbox = _new_box7(gt_cur, gt[img_id][0])
        else:
            bbox = gt[img_id]
    ...
    for img_id in pred.keys():
        ...
        for box, score in pred[img_id]:
            ...
            pred_cur[box_idx] = _box_tensor_to_7d(box.tensor)
            box_idx += 1
        pred_cur = _new_box7(pred_cur, box)
