...
class BaseApiHandler(RequestHandler):
    ...
    def prepare(self):
        self.args = json_decode(self.request.body) if len(self.request.body) > 0 else {}
...
def _make_app(handlers, help_url=None) -> Application:
    ...
    all_handlers.append(("/healthcheck", TornadoHandler, dict(checker=HealthCheck(checkers=[]))))
    all_handlers.append(("/environment", TornadoHandler, dict(checker=EnvironmentDump(application={}))))
