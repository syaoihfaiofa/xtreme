...
const zh: ILocale = {
    ...
    // motion (sync mode)
    'motion-mode': '运动模式',
    'motion-static': '静止（全场景所有帧锁定一致）',
    'motion-dynamic-fixed-size': '运动 - 尺寸固定',
    'motion-dynamic-variable-size': '运动 - 尺寸可变',
    'dynamic-range-sync': '启用动态范围同步',
    'dynamic-sync-previous-frames': '向前同步帧数',
    'dynamic-sync-next-frames': '向后同步帧数',
    ...
    'sync-x-offset': '同步X偏移',
    'sync-y-offset': '同步Y偏移',
    'sync-now': '立即同步 (Ctrl+Y)',
    'sync-now-tip': '点击后根据运动模式和当前范围设置同步该物体。提示:选中该物体后随时按 Ctrl+Y（Mac 为 Cmd+Y）即可触发同步,无需打开此面板。',
    'msg-syncing': '正在同步到其他帧…',
    'msg-sync-success': '同步完成',
    'msg-sync-fail': '同步失败',
    // class
    'class-title': '标签',
    'class-tip-info':
