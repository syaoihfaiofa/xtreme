...
* **cameraModel** : Camera projection model. Use `pinhole` by default, or `fisheye` for OpenCV fisheye/equidistant projection.
* **distortion** : Fisheye distortion coefficients. Only used when `cameraModel` is `fisheye`; supported keys are `k1`, `k2`, `k3`, and `k4`.
